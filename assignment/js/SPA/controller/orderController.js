function loadCustomeCode(){
    ...
    for (let customer of customerArray){
        $("#cmbCusCode").append(`<option>${customer.CusCode}</option>`)
    }
}
...
$('#orderId').on('keyup',function(event){
    if(event.which==13){
        ...
        order.itemsC.forEach(i => {
                var TbaleRow=`<tr ><td>${i.customCode}</td><td>${i.itemCode1}</td><td>${i.itemName1}</td><td>${i.itemDes1}</td><td>${i.itemPrice1}</td><td>${i.itemsqty}</td><td>${i.Total}</td><td>${'<button class="btn btn-outline-danger" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;" id="btnRemove">remove</button>'}</td></tr>`
